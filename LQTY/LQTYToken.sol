...
contract LQTYToken is CheckContract, ILQTYToken {
    ...
    string constant internal _NAME = "BABEL";
    string constant internal _SYMBOL = "BABEL";
        ...
        public
    {
        ...
        uint airDropEntitlement = _1_MILLION.div(2); //Allocate 0.5 miillion to air dropper
        uint totalUnlocked = bountyEntitlement.add(airDropEntitlement).add(_lpRewardsEntitlement); // Allocate 3.88 million to bounty address
        _mint(_bountyAddress, totalUnlocked);

        // Allocate the remainder to the Multisig: (100 - 32 - 1.33 - 3.88) million = 62.79 million
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "LQTY: Cannot transfer tokens directly to the LQTY token contract"
        );
        require(
    ...
    function _requireRecipientIsRegisteredLC(address _recipient) internal view {
        require(lockupContractFactory.isRegisteredLockup(_recipient) || _recipient == address(0),
        "LQTYToken: recipient must be a LockupContract registered in the Factory or zero address");
    }
